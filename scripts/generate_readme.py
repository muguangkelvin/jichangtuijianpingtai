...
def main():
    providers_file = r'c:\Users\USER\Desktop\博客\jichangtuijianpingtai.xyz\data\providers.json'
    with open(providers_file, 'r', encoding='utf-8') as f:
        providers = json.load(f)

    # Sort by rank
    providers.sort(key=lambda x: x.get('rank', 999))

    readme_content = """# 🚀 2026 机场推荐平台 | 稳定翻墙机场测速与客户端配置指南
...
"""
    ...
    table_header = "| 序号 | 机场名称 | 适用场景 / 线路特点 | 专享优惠码 | 价格门槛 | 官方链接 |"
    table_divider = "| :---: | :--- | :--- | :---: | :---: | :---: |"
    readme_content += table_header + "\n" + table_divider + "\n"

    for p in providers:
        rank = p.get('rank', 999)
        name = p.get('name', '')
        coupon = p.get('coupon', '暂无')
        if coupon == '暂无优惠码':
            coupon = '暂无'
        price = p.get('priceFrom', '以结算页为准')
        suitable = p.get('suitableFor', p.get('summary', ''))
        url = p.get('inviteURL', '')
        
        row = f"| **#{rank}** | **{name}** | {suitable} | `{coupon}` | {price} | [前往官网]({url}) |"
        readme_content += row + "\n"
...
"""
    ...
    target_dir = r'c:\Users\USER\Desktop\博客\jichangtuijian'
    os.makedirs(target_dir, exist_ok=True)
    
    file_path1 = os.path.join(target_dir, 'README.md')
    with open(file_path1, 'w', encoding='utf-8') as f:
        f.write(readme_content)

    file_path2 = r'c:\Users\USER\Desktop\博客\jichangtuijianpingtai.xyz\README.md'
